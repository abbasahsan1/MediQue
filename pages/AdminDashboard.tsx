...
import { Sparkles, BarChart2, Users, Home, QrCode, CheckCircle } from 'lucide-react';

export const AdminDashboard: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  const [stats, setStats] = useState<Record<Department, QueueStats>>(queueService.getAllStats());
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStats(queueService.getAllStats());
    }, 2000); // Poll every 2s for stats
  ...
  const totalWaiting = Object.values(stats).reduce((acc, s) => acc + s.waiting, 0);
  const totalCompleted = Object.values(stats).reduce((acc, s) => acc + s.completed, 0);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500">Live Operational Overview</p>
          </div>
